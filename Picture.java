...
public class Picture extends SimplePicture
{
  ...
  /*
   * Constructor that takes file name, creates the picture w/ name FileName
   */
  ...
  /*
   * takes the width and height of picture
   */
  ...
  public void chromakey(Picture p)
  {
	  Pixel[][] pixels = this.getPixels2D();
	  Pixel[][] replace = p.getPixels2D();
	    for (int i = 0; i < pixels.length; i++) //Pixel[][] pixels
	    {
	      for (int j = 0; j < pixels[i].length; j++) //Pixel pixelObj : rowArray
	      {
	    	  if(pixels[i][j].getBlue() > pixels[i][j].getRed())
	    	  {
	    		  pixels[i][j].setBlue(replace[i][j].getBlue());
	    		  pixels[i][j].setGreen(replace[i][j].getGreen());
	    		  pixels[i][j].setRed(replace[i][j].getRed());
	    	  }
  ...
  //hide black/white picture (crybytes, msg) in current picture by changing:
  	//alpha to odd (leaving it at 255) when msg pixel is white
  	//alpha to even (moving it to 252) when msg pixel is black
  public void encode(Picture messagePict)
  {
	  ...
	  for (int row = 0; row < this.getHeight(); row++)
	  {
		  for (int col = 0; col < this.getWidth(); col++)
		  {
			  ...
			  messagePixel = messagePixels[row][col];
			  //if messagePixel is white leave alpha unchanged
			  currPixel.setAlpha(255);
			  //if the pixel is BLACK or close to it...
			  if (messagePixel.colorDistance(Color.BLACK) < 50)
			  {
				  // currPixel.setAlpha(0);
				  currPixel.setAlpha(currPixel.getAlpha() - (currPixel.getAlpha()%4));
				  count++;
			  }
		  }
	  }
  ...
  */
  //i'm using alpha instead
  public Picture decode()
  {
	  Pixel[][] pixels = this.getPixels2D();
	  //Pixel[][] pixels = currPixels;
  ...
  {
	System.out.println("Boopala Arul, Period 1, 5/9/18, Computer 30");
	//Picture beach = new Picture("src/images/caterpillar.jpg");
    //beach.explore();
    //beach.zeroBlue();
    //beach.explore();
